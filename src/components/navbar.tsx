      ...
      >
        <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <Link href="/" className="group flex items-center gap-2">
            <span className="grid size-8 place-items-center rounded-lg bg-acid font-display text-sm font-extrabold text-void transition-transform duration-300 group-hover:rotate-12">
              BS
            </span>
          ...
          <div className="hidden items-center gap-8 md:flex">
            {LINKS.map((link) => (
              <Link
                key={link.label}
                href={link.href}
                className="group relative text-[0.8rem] font-medium uppercase tracking-[0.18em] text-mist transition-colors hover:text-white"
              >
                {link.label}
              ...
              <div className="flex flex-col gap-1 px-6 py-4">
                {LINKS.map((link) => (
                  <Link
                    key={link.label}
                    href={link.href}
                    className="rounded-lg px-3 py-3 font-display text-sm font-semibold uppercase tracking-[0.18em] text-mist transition-colors hover:bg-white/5 hover:text-acid"
                  >
                    {link.label}
                  </Link>
                ))}
